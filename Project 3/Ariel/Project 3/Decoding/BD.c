...
void read_path(TreeNode *head, char* path){
	
	TreeNode *start = tree_init(0,'&');
	start = head;
	//printf("This is the frequency %d\n",head->left->freq);
	//printf("This is the frequency %d\n",start->left->freq);
	//printf("%s", path);
	char character;
	int i=0;
	...
	while(path[i] != NULL){
	character = path[i++];
	if((head->left != NULL) && (character == '0')){
		//printf("1");
		head = head->left;
	}
	else if((head->right != NULL) && (character == '1')){
		//printf("2");
		head = head->right;
	}
...
Tree *Node = tree_init(freq[r]-'0', array[r]);
			pq_enqueue(Min_Que, Node, freq[r]-'0');
	r+=2;		
		}	
	...
	read_path(Min_Que->data[0]->root, path);
